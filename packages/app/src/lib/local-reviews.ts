 ...
 * the same answer.
 *
 * A source-scanning test enforces BOTH halves of that, and both halves are
 * needed: it walks the reading side and fails if the predicate is called
 * anywhere but here, and fails again if any file up here writes down where the
 * band begins — as a numeric threshold in any of its spellings, or as the
 * shared constant those spellings come from. A second reader does not have to
 * call the predicate to become a second opinion, and a scan for the predicate's
 * name alone would watch it walk past.
 *
 * Two exclusions are deliberate and categorical rather than an allowlist, which
 * would rot the moment a file was added. The transport is excluded for the
 * reason below. Test suites are excluded because a suite asserting that the
 * transport minted an in-band id is checking transport behaviour, not teaching
 * the interface a second definition of what "local" means — nothing a suite
 * says reaches a screen.
 *
