...
- [sub] Scratch GitHub App **`revutestbed`** (App id 4354942, installation 148013892), owner `apoha-pat` (a user account, not an org), installed on the private `apoha-pat/revu-sandbox`, **NO webhooks** (poll-only): grants `contents:write` + `pull_requests:write` + `metadata:read` + `checks:read` — deliberately **one permission wider** than the real App's actual grant (`apoha-sandbox` lacks `checks:read`; `CHECKPOINT_2.md` §E.3). Named residual: check-runs 403 is never exercised on the live legs; already mitigated in code — `syncPull` catches a 403 from `getCheckRuns` and degrades to empty checks (`packages/revud/src/direct/sync.ts:339-345`, covered by `packages/revud/src/direct/sync.test.ts:254-267` and `:269-279`). Adding `checks:read` to the real App remains an outstanding owner action.
- [sub] Scratch org + install the App; seed it mirroring the M2.1 scenarios (reuse `scripts/seed-scratch.ts`, idempotent).
- [sub] One PR opened by a **real repo-collaborator account** (distinct from the App) — the org-member interleave / `canApprove` proof; `apoha-pat` is a user account, not an org, so a collaborator stands in for an org member here.
